 ...
 * PCI device. Called from mlx5_core remove_one() so the iommu
 * attachment is gone before pci_disable_sriov() fires device_del.
 * mlx5_pci_close() has already drained the cmd ring + EQs by the time
 * we get here, so no in-flight FW DMA races the detach. The domain
 * struct stays in the PF's iova_dom[] registry and is freed later by
 * mlx5_vfmig_pf_drop_iova_domains(). No-op on PFs, on untracked VFs,
 * and on a domain that's already detached.
 */
void mlx5_vfmig_vf_detach_iova_domain(struct mlx5_core_dev *vf_mdev)
{
	...
	dom = mlx5_vf_get_vfmig_iova_domain(vf_mdev);
	if (dom)
		vfmig_iova_domain_detach_dev(dom);
}

/*
 * PF-side backstop: detach the iommu_dom from every driverless VF that
 * still has one attached, before pci_disable_sriov() removes the VFs.
 * Driver-bound VFs detach their own domain from remove_one()'s tail
 * (after mlx5_pci_close drains FW DMA); this covers tracked VFs that
 * were never bound -- the canonical destination-VF case -- which have
 * no remove_one() to run that hook and whose per-VF group would
 * otherwise WARN in the iommu core at device_del. The vf_pdev->driver
 * == NULL gate ensures we never race a bound VF's still-active FW DMA.
 * Frees nothing: vfmig_iova_domain_detach_dev() sets @dev_detached so
 * the later mlx5_vfmig_pf_drop_iova_domains() just does the
 * iommu_domain_free + kfree. Mirrors vfmig_drop_iova_domains_locked()'s
 * locking.
 */
static void
vfmig_detach_unbound_iova_domains_locked(struct mlx5_vfmig_pf *vfmig)
{
	u32 i;
...
void mlx5_vfmig_pf_detach_unbound_iova_domains(struct mlx5_core_dev *pf_mdev)
{
	...
	down_read(&vfmig->lock);
	if (!vfmig->dead)
		vfmig_detach_unbound_iova_domains_locked(vfmig);
	up_read(&vfmig->lock);
	vfmig_pf_put(vfmig);
}
